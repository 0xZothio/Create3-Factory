...
contract Create3Factory is AccessControl {
    ...
    event DeployerAdded(address indexed newDeployer);
    ...
    /// @notice Updates deployer role
    /// @param _newDeployer Address of new deployer
    /// @dev Only callable by DEFAULT_ADMIN_ROLE
    function setDeployer(
        address _newDeployer
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (_newDeployer == address(0)) {
            revert InvalidAddress("Zero address not allowed");
        }
        grantRole(DEPLOYER_ROLE, _newDeployer);
        emit DeployerAdded(_newDeployer);
    }
